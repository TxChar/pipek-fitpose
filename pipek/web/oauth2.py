...
def fetch_token(name):
    token = models.beanie_client.loop.run_until_complete(
        models.OAuth2Token.find_one(
            models.OAuth2Token.name == name,
            models.OAuth2Token.user == current_user._get_current_object(),
        )
    )
...
def update_token(name, token):
    ...
    item = models.beanie_client.loop.run_until_complete(
        models.OAuth2Token.find_one(name=name, user=current_user._get_current_object())
    )
    ...
    models.beanie_client.loop.run_until_complete(item.save())
    return item
...
def create_user_google(user_info):
    ...
    models.beanie_client.loop.run_until_complete(user.save())
    return user
...
def handle_authorized_oauth2(remote, token):
    ...
    if remote.name in ["engpsu", "psu"]:
        user = models.beanie_client.loop.run_until_complete(
            models.User.find_one(models.User.username == user_info.get("username"))
        )
    elif "email" in user_info and user_info["email"]:

        user = models.beanie_client.loop.run_until_complete(
            models.User.find_one(models.User.email == user_info.get("email"))
        )
    elif "sub" in user_info:
        user = models.beanie_client.loop.run_until_complete(
            models.User.find_one(models.User.subid == user_info.get("sub"))
        )
    ...
    if token:
        ...
        models.beanie_client.loop.run_until_complete(oauth2token.save())
...
def handle_authorize(remote, token, user_info):
    ...
    user = models.beanie_client.loop.run_until_complete(
        models.User.find_one(
            beanie.Or(
                models.User.username == user_info.get("name"),
